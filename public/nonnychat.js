...
function clickSendMessage() {
    ...
    if (chatmsg.value!="") {
        ...
        //// POST the message to /chat/:chatroom
        $.post('/chat/'+room, {
            username: username
            ,
            message:m
...
function joingame(username,room) {
    ...
    if (username!="" && room !="") {
        ...
        ////___ GET chat log for room from /chat/:chatroom
        $.get('/chat/'+room, (x) => {
            console.log(x);
            for (var m in x) {
                // timestamp, username, message
                var mc = {
                    timestamp: x[m].created_at
                    , 
                    username: x[m].username
                    ,
                    message: x[m].messagetext
                }
                mc.message = formatContentMessage(mc.message);
                console.log(mc);
                vChatLog.cms.push({content:mc});
            }
        });
